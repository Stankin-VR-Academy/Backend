...
from src.schemas.room import RoomCreate, RoomDetailResponse, RoomMemberResponse, RoomMemberUpdate, RoomResponse, RoomUpdate
...
async def list_rooms(
    ...
    try:
        ...
        logger.exception(f"List rooms error: {ex}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Could not list rooms {ex}") from ex
...
async def create_room(
    ...
    try:
        ...
        logger.exception(f"Create room error: {ex}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Could not create room {ex}") from ex
...
async def get_room(room_id: str, db: AsyncSession = Depends(get_db)) -> Any:
    ...
    try:
        ...
        room_result = await db.execute(select(Room).options(selectinload(Room.created_by_user)).where(Room.id == normalized_room_id))
        room = room_result.scalar_one_or_none()
        if not room:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Room not found")

        participant_count_result = await db.execute(select(func.count(RoomMember.id)).where(RoomMember.room_id == room.id))
        ...
        logger.exception(f"Get room error: {ex}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Could not get room {ex}") from ex
...
async def update_room(
    ...
    try:
        ...
        logger.exception(f"Update room error: {ex}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Could not update room {ex}") from ex
...
async def delete_room(
    ...
    try:
        ...
        logger.exception(f"Delete room error: {ex}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Could not delete room {ex}") from ex
...
async def list_room_members(room_id: str, db: AsyncSession = Depends(get_db)) -> Any:
    ...
    try:
        ...
        members_result = await db.execute(
            select(RoomMember)
            .where(RoomMember.room_id == normalized_room_id)
            .options(selectinload(RoomMember.user))
            .order_by(RoomMember.joined_at.asc())
        )
        return members_result.scalars().all()
    except Exception as ex:
        logger.exception(f"List room members error: {ex}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Could not list room members {ex}") from ex
...
async def add_room_member(
    ...
    try:
        ...
        logger.exception(f"Add room member error: {ex}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Could not add room member {ex}",
        ) from ex
...
async def update_room_member(
    ...
    try:
        ...
        logger.exception(f"Update room member error: {ex}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Could not update room member {ex}",
        ) from ex
...
async def remove_room_member(
    ...
    try:
        ...
        logger.exception(f"Remove room member error: {ex}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Could not remove room member {ex}") from ex
